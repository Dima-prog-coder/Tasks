...
public class Solver {
    ...
    public static int indexErrorHandler(int[][] arr, int i, int j) {
        try {
            return arr[i][j];
...
// for (int d = 0; d < 8; d++) {
//        if (indexErrorHandler(arr, i + dx[d], j + dy[d]) == arr[i][j]) {
//            friendsInfo.cnt += 1;
//            ArrayList<Integer> indexList = new ArrayList<>();
//            indexList.add(i + dx[d]);
//            indexList.add(j + dy[d]);
//            friendsInfo.lstIndexes.add(indexList);
//            return findFriendly(arr, i + dx[d], j + dy[d], friendsInfo);
//        }
//    }
//        return friendsInfo;
//}